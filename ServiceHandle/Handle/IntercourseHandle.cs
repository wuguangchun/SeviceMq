...
{
    public class IntercourseHandle
    {
        ...
        public static void mq_ReceiveCompleted(object sender, ReceiveCompletedEventArgs e)
        {
            ...
            try
            {
                ...
                //根据消息标签执行相应的命令
                if (message.Label.ToLower().Trim() == "NewCustomer".ToLower())
                {
                    //接收所有完工汇报
                    reMeg = new IntercourseHelper().EditIntercourse(message.Body.ToString());
                }
                else
                {
                    //无法识别标签内容
